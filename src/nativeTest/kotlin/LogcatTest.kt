...
class LogcatTest {
    ...
    */
    ...
    @Test
    fun `start as waiting for log lines input`() =
        runTest(dispatcher) {
            dogcat.state.test {
    ...
    @Test
    fun `begin capturing when input appears`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `get log lines if subscribed before launch`() =
        runTest(dispatcher) {
            launch {
    ...
    @Test
    fun `capture all input lines without loss`() =
        runTest(dispatcher) {
            dogcat(Start.PickAllApps)
            advanceUntilIdle()
    ...
    @Test
    fun `complete previous 'lines' upon emission of new ones`() =
        runTest(dispatcher) {
            dogcat(Start.PickAllApps)
            advanceUntilIdle()
            ...
            dogcat.state.test {
                ...
                // launch {
                input.lines.test {
                    DummyLogSource.lines.forEach { awaitItem() }
                    awaitItem()
    ...
    @Test
    fun `collectors should receive respective events`() =
        runTest(dispatcher) {
            launch {
                dogcat.state.test {
                    awaitItem() shouldBe WaitingInput
    ...
    */
    ...
    @Test
    fun `log lines flow does not complete while input is active`() =
        runTest(dispatcher) {
            ...
            dogcat.state.test {
                ...
                val j = launch { items.lines.collect { println("collected") } }
                advanceUntilIdle()
                ls.emitLine("1")
    ...
    @Test
    fun `stop input consumption upon unsubscribing`() =
        runTest(dispatcher) {
            dogcat(Start.PickAllApps)
            advanceUntilIdle()
            ...
            dogcat.state.test {
                ...
                launch(UnconfinedTestDispatcher(testScheduler)) {
                    items.lines.collect { println("coll $it") }
                    println("end")
                }
    ...
    @Test
    fun `auto re-start log consumption after clearing log input`() =
        runTest(dispatcher) {
            dogcat(Start.PickAllApps)
            advanceUntilIdle()
    ...
    @Test
    fun `emit correct indices for warnings and errors`() =
        runTest {
    ...
    // Tip: If your class creates coroutines that don't complete on their own and should be canceled
    // at the end of the test, you can inject TestScope.backgroundScope instead of the TestScope
    // itself.
    ...
    @Test
    fun `exclude log levels upon filtering`() =
        runTest(dispatcher) {
            dogcat(Start.PickAllApps)
    ...
    @Test
    fun `reset to 'waiting input' if emulator breaks and re-start logcat`() =
        runTest(dispatcher) {
